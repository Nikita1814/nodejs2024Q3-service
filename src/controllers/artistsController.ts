...
export class ArtistsController {
  ...
  findUser(@Res() res: Response, @Param('id') id: string) {
    ...
    const foundArtist = this.dbService.database.artists.find(
      (artist) => artist.id === id,
    );
  ...
  create(@Body() createArtistDto: createArtistDto, @Res() res: Response) {
    if (!createArtistDto.name || !createArtistDto.hasOwnProperty('grammy')) {
      res
        .status(HttpStatus.BAD_REQUEST)
        .json({ message: 'Please provide all the fields for the Artist' });
      return;
    } else {
      const newArtist = {
        id: randomUUID(),
  ...
  update(
    ...
    if (!foundArtist) {
      ...
      const indexOfArtist = this.dbService.database.artists.findIndex(
        (artist) => artist.id === id,
      );
      ...
      res
        ...
        .json(this.dbService.database.tracks[indexOfArtist]);
      return;
    }
  }
  ...
  delete(@Res() res: Response, @Param('id') id: string) {
    ...
    this.dbService.database.tracks = this.dbService.database.tracks.map(
      (track) => {
    ...
    this.dbService.database.albums = this.dbService.database.albums.map(
      (album) => {
    ...
    const indexOfArtistInFavorites =
      this.dbService.database.favorites.artists.findIndex(
        (artistId) => artistId === id,
      );

    if (indexOfArtistInFavorites) {
      const slicedFavs = this.dbService.database.favorites.artists.splice(
        indexOfArtistInFavorites,
        1,
      );
    }
