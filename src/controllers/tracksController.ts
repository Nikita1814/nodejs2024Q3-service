...
export class TrackController {
  ...
  @Get(':id')
  findTrack(@Res() res: Response, @Param('id') id: string) {
    ...
    console.log(id);
    const foundTrack = this.dbService.database.tracks.find(
      (track) => track.id === id,
    );
  ...
  create(@Body() createTrackDto: CrateTrackDto, @Res() res: Response) {
    if (!createTrackDto.name || createTrackDto.duration) {
      res
        .status(HttpStatus.BAD_REQUEST)
        .json({ message: 'Please provide all the fields for the track' });
      return;
    } else {
      const newTrack = {
        id: randomUUID(),
  ...
  update(
    ...
    if (!foundTrack) {
      ...
      const indexOfTrack = this.dbService.database.tracks.findIndex(
        (track) => track.id === id,
      );
      ...
      res
        ...
        .json(this.dbService.database.tracks[indexOfTrack]);
      return;
    }
  }
  ...
  delete(@Res() res: Response, @Param('id') id: string) {
    ...
    const indexOfTrackInFavorites =
      this.dbService.database.favorites.tracks.findIndex(
        (trackId) => trackId === id,
      );

    if (indexOfTrackInFavorites) {
      const slicedFavs = this.dbService.database.favorites.tracks.splice(
        indexOfTrackInFavorites,
        1,
      );
    }
