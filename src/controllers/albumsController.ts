...
export class AlbumController {
  ...
  findUser(@Res() res: Response, @Param('id') id: string) {
    ...
    const foundAlbum = this.dbService.database.albums.find(
      (album) => album.id === id,
    );
  ...
  create(@Body() createAlbumDto: CreateAlbumDto, @Res() res: Response) {
    if (!createAlbumDto.name || !createAlbumDto.year) {
      res
        .status(HttpStatus.BAD_REQUEST)
        .json({ message: 'Please provide all the fields for the album' });
      return;
    } else {
      const newAlbum = {
        id: randomUUID(),
  ...
  update(
    ...
    if (!foundAlbum) {
      ...
      const indexOfAlbum = this.dbService.database.albums.findIndex(
        (album) => album.id === id,
      );
      ...
      res
        ...
        .json(this.dbService.database.tracks[indexOfAlbum]);
      return;
    }
  }
  ...
  delete(@Res() res: Response, @Param('id') id: string) {
    ...
    const indexOfAlbumsInFavorites =
      this.dbService.database.favorites.albums.findIndex(
        (albumId) => albumId === id,
      );

    if (indexOfAlbumsInFavorites) {
      const slicedFavs = this.dbService.database.favorites.albums.splice(
        indexOfAlbumsInFavorites,
        1,
      );
    }
