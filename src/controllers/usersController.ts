...
export class UserController {
  ...
  findUser(@Res() res: Response, @Param('id') id: string) {
    if (!this.uuidRegex.test(id) || !id) {
      res
        ...
        .json({ message: 'the user id sent is not a valid UUID' });
      return;
    }

    const foundUser = this.dbService.database.users.find(
      (user) => user.id === id,
    );
  ...
  create(@Body() createUserDto: CreateUserDto, @Res() res: Response) {
    if (!createUserDto.login || !createUserDto.password) {
      res
        .status(HttpStatus.BAD_REQUEST)
        .json({ message: 'Please provide a login and password for the user' });
      return;
    } else {
      const newUser = {
        id: randomUUID(),
  ...
  update(
    ...
    if (!this.uuidRegex.test(id) || !id) {
      ...
      res.status(HttpStatus.BAD_REQUEST).json({
        message: 'Please provide the old and new password for the user',
      });
      return;
    }
    const foundUser = this.dbService.database.users.find(
    ...
    const indexOfUser = this.dbService.database.users.findIndex(
      (user) => user.id === id,
    );

    if (
      !(
        this.dbService.database.users[indexOfUser].password ===
      ...
      const user = this.dbService.database.users[indexOfUser];
      this.dbService.database.users[indexOfUser].password =
